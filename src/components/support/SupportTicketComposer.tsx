...
import type { CreateTicketPayload, TicketCategory, TicketPriority, SupportProduct } from "@/lib/types/support";
...
export function SupportTicketComposer({ onSubmit, onCancel, defaultProduct, loading }: SupportTicketComposerProps) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [category, setCategory] = useState<TicketCategory>("help_request");
  const [priority, setPriority] = useState<TicketPriority>("medium");
  const [product, setProduct] = useState<SupportProduct>(defaultProduct || "streamline-edu");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !description.trim()) return;
    await onSubmit({ title: title.trim(), description: description.trim(), category, priority, product });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-5 rounded-lg border border-border bg-card p-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
        {!defaultProduct && (
          <div className="space-y-2">
            <Label>Product</Label>
            <Select value={product} onValueChange={v => setProduct(v as SupportProduct)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {SUPPORTED_PRODUCTS.map(p => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        )}
