...
export function SupportTicketList({ tickets, onSelect, loading, emptyMessage = "No tickets found" }: SupportTicketListProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="rounded-lg border border-border overflow-hidden">
      {/* Table header */}
      <div className="hidden lg:grid lg:grid-cols-[100px_1fr_110px_140px_100px_100px_110px_120px] gap-2 px-4 py-2.5 border-b border-border bg-muted/50 text-xs font-medium text-muted-foreground">
        <span>Ticket #</span>
        <span>Title</span>
        <span>Product</span>
      ...
      {tickets.map(ticket => (
        <button
          ...
          className="w-full text-left border-b border-border last:border-b-0 px-4 py-3 transition-colors hover:bg-muted/40 focus:outline-none focus:bg-muted/40"
        >
          {/* Desktop row */}
          <div className="hidden lg:grid lg:grid-cols-[100px_1fr_110px_140px_100px_100px_110px_120px] gap-2 items-center">
            ...
            <span className="text-xs text-muted-foreground truncate">{ticket.assignedToName || "—"}</span>
